...
def contact(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()  # Save the form data as a ContactMessage object
...
def send_marks_intimation(request):
    students = Student.objects.all()
    for student in students:
        # send email to student
        subject = 'Marks Intimation'
        message = 'Students your marks for the recent exam are updated.'
        email_from = settings.EMAIL_HOST_USER
        recipient_list = [student.email_id]
        #print(recipient_list)
        send_mail(subject, message, 'rajasrees303@gmail.com',recipient_list, fail_silently= False)
    return render(request, 'info/intimation_sent.html')


def send_marks_updation(request):
    teachers = Teacher.objects.all()
    for teacher in teachers:
        # send email to student
        subject = 'Updation Intimation'
        message = 'Update the marks of students in time.'
        email_from = settings.EMAIL_HOST_USER
        recipient_list =['revathymuthukumar68@gmail.com']
        #print(recipient_list)
        send_mail(subject, message, 'sreeja20072002@gmail.com',recipient_list, fail_silently= False)
    return render(request, 'info/intimation_sent.html')
